...
3. **Classify:**
   ...
   - **External skill** -- the SKILL.md lives in a different repo (e.g., personal dotfiles, team plugins). **Never edit external skills directly.** Always generate a handoff prompt instead.
...
- Be self-contained so another agent can apply it without this session context
...
**New skills default to the local project.** Only propose creating a skill in an external repo if the skill is clearly cross-project and not specific to the current codebase -- and in that case, generate a handoff prompt instead of creating it directly.
...
1. **Scan for uncaptured entities**: Compare entities mentioned in this session against the knowledge index. Flag any that were discussed substantively but have no entry.

2. **Suggest new topic files**: If session content does not fit cleanly into existing knowledge files, propose a new topic file. Include:
   - Proposed filename and topic description
   - 2-3 seed entities that would go in it
   - Why existing files are not the right home

3. **Identify missing entity types or relationship types**: If the session surfaced entities or relationships that do not fit existing categories, propose additions to the taxonomy.

4. **Check coverage map staleness**: Scan the Coverage Map section of `index.md` for:
   - Context files that exist in `context/` but are not listed in the coverage map
